...
select 
    ...
    total_hires,
    total_terminations,
    round((total_terminations/total_hires)*100,2) as terminated_rate
from(
		select gender,
...
select 
       ...
       total_hires,
       total_terminations,
       round((total_hires/total_terminations)*100,2) as termination_rate
from( select age,
	  count(*) as total_hires,
      count(case
	            when termdate is not null and termdate>=curdate() then 1
                end) as tottal_terminations
	  from hr
